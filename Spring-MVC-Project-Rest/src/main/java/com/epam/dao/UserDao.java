...
public class UserDao {
  ...
  public User insertUser(User user) {
    ...
    NavigableSet<PhoneNumber> phoneNumberSet = user.getPhoneNumbers();
    String phoneNumbers = "";
  ...
  public boolean updateUser(User user) {
    ...
    control =
        jdbcTemplate.update(
            "UPDATE users SET username = ?, firstName = ?, lastName = ?, phoneNumbers = ?, address = ?, password = ?, roles = ? WHERE userId = ? ",
            username,
            firstName,
            lastName,
            phoneNumbers,
  ...
  public User getUserById(Long id) {
    String sql = "SELECT * FROM users WHERE userId=" + Long.toString(id);
    try {
      User resultUser =
          jdbcTemplate.queryForObject(
              sql,
  ...
  public boolean deleteUser(User user) {
    ...
    control = jdbcTemplate.update("DELETE FROM users WHERE userId = ?", id);
    if (control != -1) {
      return true;
    }
    return false;
